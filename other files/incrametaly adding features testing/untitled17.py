...
#     * Home-made linear regression AND exponential regression
#
# Plots:
#   * Takeoff profile (altitude & battery vs time, interpolated)
#   * Max range vs payload (with exponential fit)
#   * Battery endurance vs payload (with exponential fit)
...
# ----------------------------
# Utility: custom exponential "interpolation"/fit
# ----------------------------
...
def exponential_fit(x, y):
    ...
    Points with y <= 0 are ignored for the log transform.
    """
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
