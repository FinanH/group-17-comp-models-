...
usable_frac = 0.8       # We don't want to drain it 100% in real life
...
m_payload_nom = 0.5     # nominal payload for baseline take-off (kg)
...
rho = 1.225
C_d = 1.0
A_top = 0.175674        # frontal area-ish
A_disk = 1.34           # rotor disk area
eta = 0.75              # efficiency fudge
P_av = 12.0             # avionics + misc power (W), always on
...
def takeoff_dynamics(t, y):
    ...
    if E <= 0:
        # If we somehow run out of energy, cut the power (no negative energy)
        P_total = 0.0

    # A small net upward force so we actually climb
    surplus = 0.1 * m * g
    az = surplus / m

    dzdt = vz
    dvzdt = az
    dEdt = -P_total        # Energy is being consumed
...
def landing_dynamics(t, y):
    ...
    # Now we bias the acceleration downwards to descend
    F_net = -0.1 * m * g
    az = F_net / m
